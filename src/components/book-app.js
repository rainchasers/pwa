...
class BookApp extends connect(store)(LitElement) {
    ...
    return [
      css`
        :host {
          ...
          --app-main-content-min-height: calc(
            100vh - var(--app-header-height) - var(--app-footer-height) + 1px
          );
  ...
  render() {
    ...
    const hideInput =
      !_page || _page === "favorites" || _page === "about" || _page === "404";
    ...
    return html`
      ...
      <app-header condenses reveals effects="waterfall">
        <app-toolbar class="toolbar-top">
          ...
          <button
            class="signin-btn"
            aria-label="Sign In"
            ?visible="${_authInitialized}"
            @click="${() =>
              store.dispatch(_user && _user.imageUrl ? signOut() : signIn())}"
          >
            ${_user && _user.imageUrl
              ? html`
                  <img src="${_user.imageUrl}" />
                `
              : accountIcon}
          </button>
        </app-toolbar>
      ...
      <app-drawer
        ...
        ?hidden="${!_lazyResourcesLoaded}"
        @opened-changed="${e =>
          store.dispatch(updateDrawerState(e.target.opened))}"
      >
        <nav
          class="drawer-list"
          @click="${e => store.dispatch(updateDrawerState(false))}"
        >
          <a ?selected="${_page === "explore"}" href="/explore?q=${query}"
            >Home</a
          >
          <a ?selected="${_page === "favorites"}" href="/favorites"
            >Favorites</a
          >
  ...
  stateChanged(state) {
    ...
    this._authInitialized = true; // TODO: this was state.auth.initialized
    this._user = state.firebase.user;
